...
import org.attendanceApp.application.entities.Student;
import org.attendanceApp.application.verification.AttendanceConflict;
import org.attendanceApp.application.verification.ConflictDetector;
...
public class ConflictAnalysisController {
  ...
  public Node getView() {
    ...
    involvedCol.setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(
        cellData.getValue().getInvolvedStudent() != null ? cellData.getValue().getInvolvedStudent().getName() : "N/A"));
    ...
    suspNameCol
        .setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getName()));

    TableColumn<Student, String> suspIdCol = new TableColumn<>("Student ID");
    suspIdCol
        .setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getId()));
      ...
      if (allConflicts.isEmpty() && suspiciousStudents.isEmpty()) {
        ...
        summaryLabel.setText(LanguageManager.getInstance().get("conflict_detected_summary", allConflicts.size())
            ...
            + (allConflicts.isEmpty() ? ""
                : " - " + LanguageManager.getInstance().get("conflict_rate",
