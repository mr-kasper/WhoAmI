...
public class ResultsController {
  ...
  private TableView<AttendanceResultRow> createResultsTable() {
    ...
    nameCol.setCellValueFactory(
        cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().studentName));
    nameCol.setPrefWidth(120);

    // Status column
    statusCol = new TableColumn<>(LanguageManager.getInstance().get("status_col"));
    statusCol
        .setCellValueFactory(cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().status));
